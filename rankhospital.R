rankhospital <- function(state, outcome, num="best") {
        ...
        if (num == "best") {
                ...
                if (outcome == "heart attack") {
                        tt <- nrow(interested_only[!is.na(interested_only$HeartAttack),] )
                } else if (outcome == "heart failure") {
                        tt <- nrow(interested_only[!is.na(interested_only$HeartFailure),] )
                } else if (outcome == "pneumonia") {
                        tt <- nrow(interested_only[!is.na(interested_only$Pneumonia),] )
                }
                ret_Value <- interested_only[tt, "Name"]
        } else if (num > nrow(interested_only)) {
                ret_Value <- NA
        } else {
                ret_Value <- interested_only[num, "Name"]
        }
